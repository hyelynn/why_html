...
$(document).ready(function(){
    ...
    $('#price').append(
        $("<p class='font-weight-light text-gray font-size-l'>총 <span class='font-weight-bold'>"+items.length+"</span></p>"+
        ...
        "<h1 class='font-weight-bold text-red'>"+total_price+"원</h1></div>"
    ));

    $('#user').append(
        $("<div class='d-md-flex'><div class='flex-fill w-25 mr-md-4 mb-3 mb-md-0'>"+
        "<label for='order_name' class='d-block text-gray'>이름</label>"+
        "<input type='text' class='form-control' id='order_name' value='"+sessionStorage.getItem("nickname")+"' placeholder='이름'></div>"+
        "<div class='flex-fill w-75'><label for='order_email' class='d-block text-gray'>이메일</label>"+
        "<input type='text' class='form-control' id='order_email' value='"+sessionStorage.getItem("id")+"' placeholder='example@example.com'></div></div>"
    ));
})
